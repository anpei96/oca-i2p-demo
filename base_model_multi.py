...
class baseI2P(nn.Module):
    ...
    def genenrate_label_next(self, output_dict):
        ...
        output_dict["pcd_node_knn_indices_next"] = pcd_node_knn_indices
        output_dict["pcd_node_knn_masks_next"]   = pcd_node_knn_masks
        ...
        pcd_padded_pixels_f = \
            torch.cat([output_dict["pcd_pixels_f_next"], torch.ones_like(output_dict["pcd_pixels_f_next"][:1]) * 1e10], dim=0)
        pcd_node_knn_pixels = index_select(pcd_padded_pixels_f, pcd_node_knn_indices, dim=0)
        output_dict["pcd_node_masks_next"] = pcd_node_masks
        ...
        for i in range(self.img_num_levels_c):
            (
                ...
                output_dict["img_points_f_next"],
                output_dict["img_pixels_f_next"],
                output_dict["img_masks_f_next"],
                output_dict["img_h_f"],
                output_dict["img_w_f"],
                img_h_c,
                img_w_c,
            ...
            output_dict["all_img_node_knn_points_next"]  = all_img_node_knn_points
            output_dict["all_img_node_knn_pixels_next"]  = all_img_node_knn_pixels
            output_dict["all_img_node_knn_indices_next"] = all_img_node_knn_indices
            output_dict["all_img_total_nodes_next"] = all_img_total_nodes

            # print("next---img_node_knn_points: ", img_node_knn_points.size())
            # print("next---pcd_node_knn_points: ", pcd_node_knn_points.size())
            ...
            (
                ...
                pcd_node_knn_pixels,
                pcd_node_knn_masks,
                output_dict["transform_next"],
                self.matching_radius_2d,
                self.matching_radius_3d,
            )
            # print("transform_next: ", output_dict["transform_next"])
        ...
        output_dict["img_num_nodes_next"] = total_img_num_nodes
        output_dict["pcd_num_nodes_next"] = output_dict["pcd_points_c"].shape[0]
        output_dict["img_node_masks_next"]  = img_node_masks
        output_dict["img_node_levels_next"] = img_node_levels
        ...
        output_dict["gt_img_node_corr_indices_next"] = gt_img_node_corr_indices
        output_dict["gt_pcd_node_corr_indices_next"] = gt_pcd_node_corr_indices
        output_dict["gt_img_node_corr_overlaps_next"] = gt_img_node_corr_overlaps
        output_dict["gt_pcd_node_corr_overlaps_next"] = gt_pcd_node_corr_overlaps
        output_dict["gt_img_node_corr_levels_next"] = gt_img_node_corr_levels
        output_dict["gt_node_corr_min_overlaps_next"] = gt_node_corr_min_overlaps
        output_dict["gt_node_corr_max_overlaps_next"] = gt_node_corr_max_overlaps
        
        return output_dict

    def genenrate_label_prev(self, output_dict):
         # 4.1 Generate 3d patches
        _, pcd_node_sizes, pcd_node_masks, pcd_node_knn_indices, pcd_node_knn_masks = point_to_node_partition(
            output_dict["pcd_points_f"],
            output_dict["pcd_points_c"],
        ...
        output_dict["pcd_node_knn_indices_prev"] = pcd_node_knn_indices
        output_dict["pcd_node_knn_masks_prev"]   = pcd_node_knn_masks

        pcd_node_masks = torch.logical_and(pcd_node_masks, torch.gt(pcd_node_sizes, self.pcd_min_node_size))
        pcd_padded_points_f = torch.cat([output_dict["pcd_points_f"], torch.ones_like(output_dict["pcd_points_f"][:1]) * 1e10], dim=0)
        pcd_node_knn_points = index_select(pcd_padded_points_f, pcd_node_knn_indices, dim=0)
        pcd_padded_pixels_f = \
            torch.cat([output_dict["pcd_pixels_f_prev"], torch.ones_like(output_dict["pcd_pixels_f_prev"][:1]) * 1e10], dim=0)
        pcd_node_knn_pixels = index_select(pcd_padded_pixels_f, pcd_node_knn_indices, dim=0)
        output_dict["pcd_node_masks_prev"] = pcd_node_masks
        ...
        for i in range(self.img_num_levels_c):
            (
                ...
                output_dict["img_points_f_prev"],
                output_dict["img_pixels_f_prev"],
                output_dict["img_masks_f_prev"],
                output_dict["img_h_f"],
                output_dict["img_w_f"],
                img_h_c,
                img_w_c,
            ...
            output_dict["all_img_node_knn_points_prev"]  = all_img_node_knn_points
            output_dict["all_img_node_knn_pixels_prev"]  = all_img_node_knn_pixels
            output_dict["all_img_node_knn_indices_prev"] = all_img_node_knn_indices
            output_dict["all_img_total_nodes_prev"] = all_img_total_nodes

            # print("prev---img_node_knn_points: ", img_node_knn_points.size())
            # print("prev---pcd_node_knn_points: ", pcd_node_knn_points.size())
            ...
            (
                ...
                pcd_node_knn_pixels,
                pcd_node_knn_masks,
                output_dict["transform_prev"],
                self.matching_radius_2d,
                self.matching_radius_3d,
            )
            # print("transform_prev: ", output_dict["transform_prev"])
        ...
        output_dict["img_num_nodes_prev"] = total_img_num_nodes
        output_dict["pcd_num_nodes_prev"] = output_dict["pcd_points_c"].shape[0]
        output_dict["img_node_masks_prev"]  = img_node_masks
        output_dict["img_node_levels_prev"] = img_node_levels
        ...
        output_dict["gt_img_node_corr_indices_prev"] = gt_img_node_corr_indices
        output_dict["gt_pcd_node_corr_indices_prev"] = gt_pcd_node_corr_indices
        output_dict["gt_img_node_corr_overlaps_prev"] = gt_img_node_corr_overlaps
        output_dict["gt_pcd_node_corr_overlaps_prev"] = gt_pcd_node_corr_overlaps
        output_dict["gt_img_node_corr_levels_prev"] = gt_img_node_corr_levels
        output_dict["gt_node_corr_min_overlaps_prev"] = gt_node_corr_min_overlaps
        output_dict["gt_node_corr_max_overlaps_prev"] = gt_node_corr_max_overlaps
    ...
    def sub_func_for_2d_data_unpack(self, depth, intrinsics, img_h, img_w):
        ...
        return img_points, img_pixels, img_masks, img_points_f, img_pixels_f, img_masks_f
    ...
    def unpack_2d_3d_data(self, data_dict, output_dict):
        ...
        '''
        # revise a bug in data_dict
        data_dict["image_next"] = data_dict["image_next"].unsqueeze(0)
        data_dict["image_prev"] = data_dict["image_prev"].unsqueeze(0)
        data_dict["depth_next"] = data_dict["depth_next"].unsqueeze(0)
        data_dict["depth_prev"] = data_dict["depth_prev"].unsqueeze(0)

        # 2d image branch
        image      = data_dict["image"].unsqueeze(1).detach() 
        image      = image.transpose(1, -1)
        image      = image.squeeze(-1)
        image_next = data_dict["image_next"].unsqueeze(1).detach() 
        image_next = image_next.transpose(1, -1)
        image_next = image_next.squeeze(-1)
        image_prev = data_dict["image_prev"].unsqueeze(1).detach() 
        image_prev = image_prev.transpose(1, -1)
        image_prev = image_prev.squeeze(-1)

        depth          = data_dict["depth"].detach()       # (B, H, W)
        depth_next     = data_dict["depth_next"].detach()  # (B, H, W)
        depth_prev     = data_dict["depth_prev"].detach()  # (B, H, W)
        intrinsics     = data_dict["intrinsics"].detach()  # (B, 3, 3)
        transform      = data_dict["transform"].detach()
        transform_next = data_dict["transform_next"].detach()
        transform_prev = data_dict["transform_prev"].detach()
        # print("depth:      ", depth)
        # print("depth_next: ", depth_next)
        # print("depth_prev: ", depth_prev)
        ...
        output_dict["transform_next"] = transform_next
        output_dict["transform_prev"] = transform_prev
        ...
        output_dict["img_pixels_c_next"] = img_pixels_c
        output_dict["img_pixels_c_prev"] = img_pixels_c

        img_points  , img_pixels  , img_masks, \
        img_points_f, img_pixels_f, img_masks_f \
            = self.sub_func_for_2d_data_unpack(depth, intrinsics, img_h, img_w)
        ...
        # print("image_next: ", image_next.size())
        # print("image_prev: ", image_prev.size())

        # print("depth:      ", depth.size())
        # print("depth_next: ", depth_next.size())
        # print("depth_prev: ", depth_prev.size())
        # assert 1==-1

        img_points_next  , img_pixels_next  , img_masks_next, \
        img_points_f_next, img_pixels_f_next, img_masks_f_next \
            = self.sub_func_for_2d_data_unpack(depth_next, intrinsics, img_h, img_w)
        output_dict["img_points_next"]   = img_points_next
        output_dict["img_pixels_next"]   = img_pixels_next
        output_dict["img_masks_next"]    = img_masks_next
        output_dict["img_points_f_next"] = img_points_f_next
        output_dict["img_pixels_f_next"] = img_pixels_f_next
        output_dict["img_masks_f_next"]  = img_masks_f_next

        img_points_prev  , img_pixels_prev  , img_masks_prev, \
        img_points_f_prev, img_pixels_f_prev, img_masks_f_prev \
            = self.sub_func_for_2d_data_unpack(depth_prev, intrinsics, img_h, img_w)
        output_dict["img_points_prev"]   = img_points_prev
        output_dict["img_pixels_prev"]   = img_pixels_prev
        output_dict["img_masks_prev"]    = img_masks_prev
        output_dict["img_points_f_prev"] = img_points_f_prev
        output_dict["img_pixels_f_prev"] = img_pixels_f_prev
        output_dict["img_masks_f_prev"]  = img_masks_f_prev
        ...
        pcd_pixels_f_next = \
            render(pcd_points_f, intrinsics, extrinsics=transform_next, rounding=False)
        pcd_pixels_f_prev = \
            render(pcd_points_f, intrinsics, extrinsics=transform_prev, rounding=False)
        ...
        output_dict["pcd_pixels_f_next"] = pcd_pixels_f_next
        output_dict["pcd_pixels_f_prev"] = pcd_pixels_f_prev

        return image, image_next, image_prev, output_dict
    ...
    def post_process_generate_corres_next(self, img_feats_c, pcd_feats_c,
        img_feats_f, pcd_feats_f, output_dict):
        (
            img_node_corr_indices,
            pcd_node_corr_indices,
            node_corr_scores,
        ) = self.coarse_matching(img_feats_c, pcd_feats_c, output_dict["img_node_masks_next"], output_dict["pcd_node_masks_next"])
        img_node_corr_levels = output_dict["img_node_levels_next"][img_node_corr_indices]

        output_dict["img_node_corr_indices_next"] = img_node_corr_indices
        output_dict["pcd_node_corr_indices_next"] = pcd_node_corr_indices
        output_dict["img_node_corr_levels_next"] = img_node_corr_levels
        ...
        for i in range(self.img_num_levels_c):
            ...
            cur_img_node_corr_indices = img_node_corr_indices[node_corr_masks] - output_dict["all_img_total_nodes_next"][i]
            cur_pcd_node_corr_indices = pcd_node_corr_indices[node_corr_masks]

            img_node_knn_points  = output_dict["all_img_node_knn_points_next"][i]
            img_node_knn_pixels  = output_dict["all_img_node_knn_pixels_next"][i]
            img_node_knn_indices = output_dict["all_img_node_knn_indices_next"][i]

            img_node_corr_knn_indices = index_select(img_node_knn_indices, cur_img_node_corr_indices, dim=0)
            img_node_corr_knn_masks = torch.ones_like(img_node_corr_knn_indices, dtype=torch.bool)
            img_node_corr_knn_feats = index_select(img_feats_f, img_node_corr_knn_indices, dim=0)

            pcd_node_corr_knn_indices = output_dict["pcd_node_knn_indices_next"][cur_pcd_node_corr_indices]  # (P, Kc)
            pcd_node_corr_knn_masks = output_dict["pcd_node_knn_masks_next"][cur_pcd_node_corr_indices]  # (P, Kc)
            pcd_node_corr_knn_feats = index_select(pcd_padded_feats_f, pcd_node_corr_knn_indices, dim=0)
        ...
        img_points_f = output_dict["img_points_f_next"].view(-1, 3)
        img_pixels_f = output_dict["img_pixels_f_next"].view(-1, 2)
        img_corr_points = img_points_f[img_corr_indices]
        img_corr_pixels = img_pixels_f[img_corr_indices]
        pcd_corr_points = output_dict["pcd_points_f"][pcd_corr_indices]
        pcd_corr_pixels = output_dict["pcd_points_f"][pcd_corr_indices]
        ...
        output_dict["img_corr_points_next"]  = img_corr_points
        output_dict["img_corr_pixels_next"]  = img_corr_pixels
        output_dict["img_corr_indices_next"] = img_corr_indices
        output_dict["pcd_corr_points_next"]  = pcd_corr_points
        output_dict["pcd_corr_pixels_next"]  = pcd_corr_pixels
        output_dict["pcd_corr_indices_next"] = pcd_corr_indices
        output_dict["corr_scores_next"]      = corr_scores
        return output_dict

    def post_process_generate_corres_prev(self, img_feats_c, pcd_feats_c,
        img_feats_f, pcd_feats_f, output_dict):
        (
            img_node_corr_indices,
            pcd_node_corr_indices,
            node_corr_scores,
        ) = self.coarse_matching(img_feats_c, pcd_feats_c, output_dict["img_node_masks_prev"], output_dict["pcd_node_masks_prev"])
        img_node_corr_levels = output_dict["img_node_levels_prev"][img_node_corr_indices]

        output_dict["img_node_corr_indices_prev"] = img_node_corr_indices
        output_dict["pcd_node_corr_indices_prev"] = pcd_node_corr_indices
        output_dict["img_node_corr_levels_prev"] = img_node_corr_levels
        ...
        for i in range(self.img_num_levels_c):
            ...
            cur_img_node_corr_indices = img_node_corr_indices[node_corr_masks] - output_dict["all_img_total_nodes_prev"][i]
            cur_pcd_node_corr_indices = pcd_node_corr_indices[node_corr_masks]

            img_node_knn_points  = output_dict["all_img_node_knn_points_prev"][i]
            img_node_knn_pixels  = output_dict["all_img_node_knn_pixels_prev"][i]
            img_node_knn_indices = output_dict["all_img_node_knn_indices_prev"][i]

            img_node_corr_knn_indices = index_select(img_node_knn_indices, cur_img_node_corr_indices, dim=0)
            img_node_corr_knn_masks = torch.ones_like(img_node_corr_knn_indices, dtype=torch.bool)
            img_node_corr_knn_feats = index_select(img_feats_f, img_node_corr_knn_indices, dim=0)

            pcd_node_corr_knn_indices = output_dict["pcd_node_knn_indices_prev"][cur_pcd_node_corr_indices]  # (P, Kc)
            pcd_node_corr_knn_masks = output_dict["pcd_node_knn_masks_prev"][cur_pcd_node_corr_indices]  # (P, Kc)
            pcd_node_corr_knn_feats = index_select(pcd_padded_feats_f, pcd_node_corr_knn_indices, dim=0)
        ...
        img_points_f = output_dict["img_points_f_prev"].view(-1, 3)
        img_pixels_f = output_dict["img_pixels_f_prev"].view(-1, 2)
        img_corr_points = img_points_f[img_corr_indices]
        img_corr_pixels = img_pixels_f[img_corr_indices]
        pcd_corr_points = output_dict["pcd_points_f"][pcd_corr_indices]
        pcd_corr_pixels = output_dict["pcd_points_f"][pcd_corr_indices]
        ...
        output_dict["img_corr_points_prev"]  = img_corr_points
        output_dict["img_corr_pixels_prev"]  = img_corr_pixels
        output_dict["img_corr_indices_prev"] = img_corr_indices
        output_dict["pcd_corr_points_prev"]  = pcd_corr_points
        output_dict["pcd_corr_pixels_prev"]  = pcd_corr_pixels
        output_dict["pcd_corr_indices_prev"] = pcd_corr_indices
        output_dict["corr_scores_prev"]      = corr_scores
        return output_dict
